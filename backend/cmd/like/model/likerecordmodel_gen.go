...
var (
	...
	cacheLikeRecordBizIdObjIdUserIdPrefix = "cache:likeRecord:bizId:objId:userId:"
)
...
func (m *defaultLikeRecordModel) Delete(ctx context.Context, id int64) error {
	...
	likeRecordBizIdObjIdUserIdKey := fmt.Sprintf("%s%v:%v:%v", cacheLikeRecordBizIdObjIdUserIdPrefix, data.BizId, data.ObjId, data.UserId)
	likeRecordIdKey := fmt.Sprintf("%s%v", cacheLikeRecordIdPrefix, id)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, likeRecordBizIdObjIdUserIdKey, likeRecordIdKey)
	return err
}
...
func (m *defaultLikeRecordModel) FindOneByBizIdObjIdUserId(ctx context.Context, bizId string, objId int64, userId int64) (*LikeRecord, error) {
	likeRecordBizIdObjIdUserIdKey := fmt.Sprintf("%s%v:%v:%v", cacheLikeRecordBizIdObjIdUserIdPrefix, bizId, objId, userId)
	var resp LikeRecord
	err := m.QueryRowIndexCtx(ctx, &resp, likeRecordBizIdObjIdUserIdKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
