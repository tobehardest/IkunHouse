...
var (
	...
	cacheLikeCountIdPrefix         = "cache:likeCount:id:"
	cacheLikeCountBizIdObjIdPrefix = "cache:likeCount:bizId:objId:"
)
...
func (m *defaultLikeCountModel) Delete(ctx context.Context, id int64) error {
	...
	likeCountBizIdObjIdKey := fmt.Sprintf("%s%v:%v", cacheLikeCountBizIdObjIdPrefix, data.BizId, data.ObjId)
	likeCountIdKey := fmt.Sprintf("%s%v", cacheLikeCountIdPrefix, id)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, likeCountBizIdObjIdKey, likeCountIdKey)
	return err
}
...
func (m *defaultLikeCountModel) FindOne(ctx context.Context, id int64) (*LikeCount, error) {
	likeCountIdKey := fmt.Sprintf("%s%v", cacheLikeCountIdPrefix, id)
	var resp LikeCount
	err := m.QueryRowCtx(ctx, &resp, likeCountIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultLikeCountModel) FindOneByBizIdObjId(ctx context.Context, bizId string, objId int64) (*LikeCount, error) {
	likeCountBizIdObjIdKey := fmt.Sprintf("%s%v:%v", cacheLikeCountBizIdObjIdPrefix, bizId, objId)
	var resp LikeCount
	err := m.QueryRowIndexCtx(ctx, &resp, likeCountBizIdObjIdKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
...
func (m *defaultLikeCountModel) Insert(ctx context.Context, data *LikeCount) (sql.Result, error) {
	likeCountBizIdObjIdKey := fmt.Sprintf("%s%v:%v", cacheLikeCountBizIdObjIdPrefix, data.BizId, data.ObjId)
	likeCountIdKey := fmt.Sprintf("%s%v", cacheLikeCountIdPrefix, data.Id)
...
func (m *defaultLikeCountModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheLikeCountIdPrefix, primary)
}
